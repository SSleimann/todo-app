...
async def create_task_todo(
    body: TaskCreationDTO, service: ToDoService = Depends(get_service_todo)
):
...
async def update_put(
    body: TaskUpdatePutDTO, service: ToDoService = Depends(get_service_todo)
):
...
async def update_patch(
    body: TaskUpdatePatchDTO, service: ToDoService = Depends(get_service_todo)
):
    result = await service.update(body)
